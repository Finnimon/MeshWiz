...
public readonly record struct OclPlatform(IntPtr Handle) : IDisposable
{
    ...
    public Result<OclResultCode,OclDevice[]> Gpus => GetDevices(this, DeviceType.Gpu);
    public Result<OclResultCode,OclDevice[]> Cpus => GetDevices(this, DeviceType.Cpu);
    public Result<OclResultCode,OclDevice[]> AcceleratorDevices => GetDevices(this, DeviceType.Accelerator);
    public Result<OclResultCode,OclDevice[]> CustomDevices => GetDevices(this, DeviceType.Custom);
    public Result<OclResultCode,OclDevice[]> AllDevices => GetDevices(this, DeviceType.All);
    public Result<OclResultCode,string> Name => GetName(this);

    public static Result<OclResultCode,string> GetName(CLPlatform platform) =>
        GetInfo(platform, PlatformInfo.Name)
            .Select(OclHelper.GetCLString);

    public static Result<OclResultCode, byte[]> GetInfo(CLPlatform platform, PlatformInfo info)
        => CL.GetPlatformInfo(platform, info, out var data).AsResult(data);
    

    public static Result<OclResultCode,OclPlatform[]> GetAll()
    {
        return CL.GetPlatformIds(out var value).AsResult(value)
            .Select(dev => dev.Select(Create).ToArray());
    }

    public static Result<OclResultCode,OclDevice[]> GetDevices(CLPlatform obj, DeviceType deviceType)
    {
