...
public static partial class ArrayBuilder
{
    ...
    private ref struct SegmentedUnmanaged<T> : IBuilder<SegmentedUnmanaged<T>, T>
    {
        static SegmentedUnmanaged()
        {
            if (RuntimeHelpers.IsReferenceOrContainsReferences<T>())
                ThrowHelper.ThrowNotSupportedException($"{typeof(T)} is not supported. It must me unmanaged.");
        }
        ...
        public SegmentedUnmanaged(Span<T> scratchBuffer) => _currentSegment = _firstSegment = scratchBuffer;
        ...
        private void ReturnArrays(int segmentsCount)
        {
            ...
            foreach (var segment in ((ReadOnlySpan<T[]>) _segments))
            {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void AddEnumerating(IEnumerable<T> enumerable) => AddEnumeratingInlined(enumerable);
        ...
        /// </remarks>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void AddEnumeratorInlined<TIter>(TIter source)
        ...
        private void Expand(int minimumRequired = MinimumRentSize)
        {
            ...
            var newSegmentLength =
                (int)Math.Min(Math.Max(minimumRequired, currentSegmentLength * 2L), Array.MaxLength);
...
#pragma warning restore IDE0044
        ...
        public static T[] ToArrayInlined(IEnumerable<T> data)
        {
            ...
            using SegmentedUnmanaged<T> b = new(s);
    ...
    /// <remarks>This is implemented as an inline array of rented arrays.</remarks>
    /// <typeparam name="T">Specifies the element type of the collection being built.</typeparam>
    public ref struct Segmented<T> : IBuilder<Segmented<T>, T>
    {
        /// <summary>Minimum size to request renting from the pool.</summary>
        private const int MinimumRentSize = 16;
        ...
        /// <summary>Initialize the builder.</summary>
        /// <param name="scratchBuffer">A buffer that can be used as part of the builder.</param>
        public Segmented(Span<T> scratchBuffer) => _currentSegment = _firstSegment = scratchBuffer;
