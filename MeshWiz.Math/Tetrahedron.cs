    ...
    where TNum : unmanaged, IFloatingPointIeee754<TNum>
{
    ...
    {
        var abc = new Triangle3<TNum>(A, B, C).SurfaceArea;
        var dab = new Triangle3<TNum>(D, A, B).SurfaceArea;
        var cda = new Triangle3<TNum>(C, D, A).SurfaceArea;
        var bcd = new Triangle3<TNum>(B, C, D).SurfaceArea;
    ...
    public Tetrahedron<TOther> To<TOther>()
        where TOther : unmanaged, IFloatingPointIeee754<TOther>
        => new(A.To<TOther>(), B.To<TOther>(), C.To<TOther>(), D.To<TOther>());
}